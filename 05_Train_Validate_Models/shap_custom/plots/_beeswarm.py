...
#         cb.ax.set_aspect((bbox.height - 0.9) * 20)
    ...
    if midline_decimal_point:
        ...
        print(loc, xlabs)
        labs = []
        #convert value to string and replace the decimal point
        for val in loc:
            if (val < 0.0) or (val > 1.0):
                ...
                v = f'{val:.1f}'.replace('.', '·')
            labs.append(f'{v}')
        # clear label for the first and last items    
        labs[0] = ''
        labs[-1] = ''    
        # set new values with decimal point replaced
        pl.xticks(loc, labels=labs)
    ...
    if plot_type == "dot":
        ...
        for pos, ind in enumerate(feature_order):
            ...
            for i in range(nbins - 1, -1, -1):
                y = ys[i, :] / scale        # REPLACE "DECIMAL POINT" TO "MIDLINE DECIMAL POINT"
        if midline_decimal_point:
            # get x-axis values
            loc, xlabs = pl.xticks()
            # print(loc, xlabs)
